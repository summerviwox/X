...
public class AudioPlayer {
    ...
    public static int MAX_LENTHSECOND = 60;
    public static String BASE_DRI_NAME =XApp.getInstance().getResources().getString(R.string.app_name);
    private static String BASE_DRI = Environment.getExternalStorageDirectory().getPath() + "/"+ BASE_DRI_NAME +"/";
    private static String CURRENT_RECORD_FILE = BASE_DRI+ "auto_";
    ...
    public void startRecord(Callback callback) {
        ...
        try {
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    stopRecord(false);
                    ToastUtils.showLong("已达到最大语音长度");
                }
            }, MAX_LENTHSECOND * 1000);
        } catch (Exception e) {
            stopInternalRecord();
            onRecordCompleted(false,false);
        }
    }

    public void stopRecord(boolean cancle) {
        stopInternalRecord();
        onRecordCompleted(true,cancle);
    ...
    public void startPlay(String filePath, Callback callback) {
        ...
        try {
            ...
            ToastUtils.showLong("语音文件已损坏或不存在");
            stopInternalPlay();
    ...
    private void onPlayCompleted(boolean success) {
        if (mPlayCallback != null) {
            mPlayCallback.onCompletion(true,success);
        }
        mPlayer = null;
    }

    private void onRecordCompleted(boolean success,boolean cancle) {
        if (mRecordCallback != null) {
            mRecordCallback.onCompletion(success,cancle);
        }
        mRecorder = null;
    }
    ...
    public interface Callback {
        void onCompletion(boolean success, Boolean cancle);
    }
}
